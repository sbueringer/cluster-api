...
func (r *Reconciler) syncMachines(ctx context.Context, s *scope) (ctrl.Result, bool, error) {
	...
	for i := range machines {
		...
		if !anyManagedFieldIssueMitigated {
			...
			if err != nil {
				return ctrl.Result{}, true, pkgerrors.Wrapf(err, "failed to apply Machine")
			}
...
func (r *Reconciler) updateLabelsAndAnnotations(ctx context.Context, c ssa.WriterWithScheme, objGVK schema.GroupVersionKind, obj client.Object, machineSet *clusterv1.MachineSet) error {
	// FIXME: consider moving this func into the ssa package and reuse with KCP
	...
	requestIdentifier, err := ssa.ComputeRequestIdentifier(c.Scheme(), obj.GetResourceVersion(), updatedObject)
	if err != nil {
		return pkgerrors.Wrapf(err, "failed to apply labels/annotations of %s", objGVK.Kind)
	}
	if r.ssaApplyConfigurationCache.Has(requestIdentifier, objGVK.Kind) {
		// Nothing to do
		// Refresh the cache
		r.ssaApplyConfigurationCache.Add(requestIdentifier)
		return nil
