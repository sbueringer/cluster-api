...
func validateRolloutAndCertValidityFields(rolloutSpec controlplanev1.KubeadmControlPlaneRolloutSpec, clusterConfiguration *bootstrapv1.ClusterConfiguration, replicas *int32, pathPrefix *field.Path) field.ErrorList {
	...
	if clusterConfiguration.CertificateValidityPeriodDays != 0 { // If CertificateValidityPeriodDays is set
		if clusterConfiguration.CACertificateValidityPeriodDays != 0 { // If CACertificateValidityPeriodDays is set
		...
		if rolloutSpec.Before.CertificatesExpiryDays != 0 { // If CertificatesExpiryDays is set
