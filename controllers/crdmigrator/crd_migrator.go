...
// Phase is a phase of the CRD migrator.
type Phase string
...
func (r *CRDMigrator) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	// Note: CRD might have a newer generation than PartialObjectMeta above, we should use the
	...
	for _, obj := range customResourceObjects {
		...
		if err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
			...
			if len(managedFields) == 0 {
				...
				// NOTE: We use the fieldManager from the managedFields that we remove to increase the chances that the managedField entry gets cleaned up.
				//       In any case having a minimal entry only for metadata.name is better than leaving the old entry that uses an apiVersion that is not served anymore.
