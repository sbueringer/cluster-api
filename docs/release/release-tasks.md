...
- [Release Lead](#release-lead)
  ...
  - [Tasks](#tasks)
    - [Set a tentative rel2ease date for the minor release](#set-a-tentative-release-date-for-the-minor-release)
...
2. Promote images from the staging repository to the production registry (`registry.k8s.io/cluster-api`):
...
4. Publish `clusterctl` to Homebrew by bumping the version in [clusterctl.rb](https://github.com/Homebrew/homebrew-core/blob/master/Formula/clusterctl.rb).
   **Notes**:
...
5. Set EOL date for previous release (prior art: https://github.com/kubernetes-sigs/cluster-api/issues/7146).
...
3. Finalize the release notes:
    ...
    7. Add Kubernetes version support section.
4. Iterate until the GA release (the release note tool can be run starting from a specific commit).
...
1. Create new jobs based on the jobs running against our `main` branch:
    ...
    4. Modify the following:
        ...
        2. Change `extra_refs[].base_ref` to `release-1.4`.
        3. Change `annotations.testgrid-dashboards` to `sig-cluster-lifecycle-cluster-api-1.4`.
        4. Change `annotations.testgrid-tab-name`, e.g. `capi-test-main` => `capi-test-release-1-4`.
        5. For periodics additionally: Change interval (let's use the same as for `1.3`).
        6. For presubmits additionally: Adjust branches: `^main$` => `^release-1.4$`.
