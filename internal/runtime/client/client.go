...
func (c *client) GetAllExtensions(ctx context.Context, hook runtimecatalog.Hook, forObject metav1.Object) ([]string, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "failed to call extension handlers for hook %q: failed to compute GroupVersionHook", hookName)
	}

	registrations, err := c.registry.List(gvh.GroupHook())
	if err != nil {
		return nil, errors.Wrapf(err, "failed to call extension handlers for hook %q", gvh.GroupHook())
	}

	log.V(4).Info(fmt.Sprintf("Calling all extensions of hook %q", hookName))
	var matchingRegistrations []string
	for _, registration := range registrations {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "failed to call extension handlers for hook %q: failed to call extension handler %q", gvh.GroupHook(), registration.Name)
		}
