...
func TestWebhook(t *testing.T) {
	...
	if err := env.Create(ctx, testtypes.GenericControlPlaneTemplateCRD); err != nil && !apierrors.IsAlreadyExists(err) {
		panic(err)
	}
	if err := env.Create(ctx, testtypes.GenericInfrastructureClusterTemplateCRD); err != nil && !apierrors.IsAlreadyExists(err) {
		panic(err)
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			clusterClass.Spec.Variables.Definitions = toVariableClasses(g, tt.variableClasses)
	...
	return &clusterv1.ClusterClass{
		...
		Spec: clusterv1.ClusterClassSpec{
			ControlPlane: clusterv1.ControlPlaneClass{
				LocalObjectTemplate: clusterv1.LocalObjectTemplate{
					Ref: &corev1.ObjectReference{
						APIVersion: testtypes.ControlPlaneGroupVersion.String(),
			...
			Infrastructure: clusterv1.LocalObjectTemplate{
				Ref: &corev1.ObjectReference{
					APIVersion: testtypes.InfrastructureGroupVersion.String(),
...
func myControlPlane(namespace string) *unstructured.Unstructured {
	...
	obj.SetAPIVersion(testtypes.ControlPlaneGroupVersion.String())
