...
func TestMain(m *testing.M) {
	...
	for _, fg := range []featuregate.Feature{features.ClusterTopology, features.KubeadmBootstrapFormatIgnition, features.RuntimeSDK, features.InPlaceUpdates, features.MachineTaintPropagation} {
		if err := features.Gates.(featuregate.MutableFeatureGate).Set(fmt.Sprintf("%s=%v", fg, true)); err != nil {
